...
currentStart<- "2025 January"
currentIssue<-"2025T3"
currentYear<-"2025"
currentTrimester<- "November 30" #end month
```
...
#the tab names are inconsistent - seems an artefact of human db download - must manually correct to match IH_OL_data and Lab_data
#last column has Aziz (does not always) add manually using excel formula ( to sta_num_shortname) !!
...
library(scales)
p_haulageByPlant<-dt_wiskiHaul %>% 
  ggplot(aes(x= Date, y = Value_m3)) + #aes b/c will update will be based on variable
  geom_bar(stat = 'identity', aes(fill = site_Name))+
  #geom_text(aes(label=Value_m3), vjust= -0.3, size = 3.5)+
  theme_minimal()+
  scale_fill_viridis(discrete = TRUE)+
  scale_x_datetime(date_breaks = "1 month", labels = date_format("%b"))

ggplotly(p_haulageByPlant)

#try export to html ()
library(htmlwidgets)
saveWidget(ggplotly(p_haulageByPlant), file=paste("export/", currentIssue, "/p_haulageByPlant.html", sep=""))
#generating div - to try https://stackoverflow.com/questions/36262748/save-plotly-plot-to-local-file-and-insert-into-html
```
...
dt_wiskiEff<-dt_wiskiBSD %>%
  ...
  filter(grepl("Eff", StationName))%>%  #gets rid of centrifuge, polymer (Amherstburg)
  #discard all numerical as they seem to be sub meters/sub flows which get summed (will inadvertently drop places which do not combine)
...
#5302 has Flow-1, Flow1-1 and Combined Flow (Flow1+Flow2+Flow3) - combined gets scrubbed b/c of number
...
dt_wiskiHaulRegion<- left_join(dt_wiskiHaul, dt_wiskiMeta[,c(1:17)], by = c("site_no" = "orgUnit")) 

dt_wiskiHaulRegion <- dt_wiskiHaulRegion %>% group_by(RegionList) %>% #group_by(site_no) to find largest haulers 
  summarize(
    HauledCumulative_m3 = round(sum(Value_m3), digits = 0)
  )

dt_wiskiEffRegion<- left_join(dt_wiskiEff, dt_wiskiMeta[,c(1:17)], by = c("site_no" = "orgUnit"))
dt_wiskiEffRegion <- dt_wiskiEffRegion %>% group_by(RegionList) %>%
  summarize(
    EffluentCumulative_m3 = round(sum(EffluentCumulative_m3), digits = 0)
  )

dt_wiskiProdByRegion<-left_join(dt_wiskiEffRegion, dt_wiskiHaulRegion, by = "RegionList")

dt_wiskiProdByRegion$Yield_kgBiosolidPerm3Effluent<-dt_wiskiProdByRegion$HauledCumulative_m3/dt_wiskiProdByRegion$EffluentCumulative_m3*1000
```


```{r}
#find top and least producing plant; dropdown -Peel does not report haulage volumes

dt_wiskiHaulPlant<- left_join(dt_wiskiHaul, dt_wiskiMeta[,c(1:17)], by = c("site_no" = "orgUnit")) 

dt_wiskiHaulPlant <- dt_wiskiHaulPlant %>% group_by(site_Name, RegionList) %>% #group_by(site_no) to find largest haulers 
  summarize(
    HauledCumulative_m3 = round(sum(Value_m3), digits = 0),
  )

p_wiskiHaulPlant<-dt_wiskiHaulPlant %>%
  ggplot(aes(x=site_Name, y=HauledCumulative_m3)) +
    geom_bar(stat = 'identity', aes(fill = RegionList))+
    geom_text(data=dt_wiskiHaulPlant, aes(label=HauledCumulative_m3), vjust= -0.3, size = 3.5)+
    theme_minimal()+
    scale_fill_viridis(discrete = TRUE)+
    geom_jitter(color="black", size=0.4, alpha=0.9) +
    theme(axis.text.x  = element_text(angle=90, vjust=0.5, size=5), legend.position = "top", legend.title = element_blank())+
    labs(y = "Hauled Volume (m3)",
         x= "",
       title = paste("Total Hauled volume recorded in WISKI,", currentStart, "to", currentTrimester),
       caption = "South Peel quantities not available from WISKI")+
   coord_flip()+scale_x_discrete(limits = rev)

p_wiskiHaulPlant

ggsave(file=paste("export/", currentIssue, "/p_haulageByPlant.svg", sep=""), plot=p_wiskiHaulPlant, width=16, height=16)
```

```{r}
#dry tonnage
dt_wiskiLabBSD %>% group_by(site_Name) %>% 
  summarize(
    medianTS_mgPerL = median(Value_mgPerL)
  )

dt_wiskiHaulPlant<- left_join(dt_wiskiHaulPlant, 
                              dt_wiskiLabBSD[,c(1,2,12)]%>% 
                                group_by(site_Name) %>% 
                                summarize(medianTS_mgPerL = median(Value_mgPerL)), 
                              by = c("site_Name")) 

dt_wiskiHaulPlant$dryTonnage<-round(dt_wiskiHaulPlant$medianTS_mgPerL * dt_wiskiHaulPlant$HauledCumulative_m3/1000000, 2) 

p_wiskiHaulPlantDry<-dt_wiskiHaulPlant %>%
  ggplot(aes(x=site_Name, y=dryTonnage)) +
    geom_bar(stat = 'identity', aes(fill = RegionList))+
    geom_text(data=dt_wiskiHaulPlant, aes(label=dryTonnage), vjust= -0.3, size = 3.5)+
    theme_minimal()+
    scale_fill_viridis(discrete = TRUE)+
    geom_jitter(color="black", size=0.4, alpha=0.9) +
    theme(axis.text.x  = element_text(angle=90, vjust=0.5, size=5), legend.position = "top", legend.title = element_blank())+
    labs(y = "Hauled Dry Tonnage Estimate (t)",
         x= "",
       title = paste("Dry tonnage estimated by Total Hauled volume and median biosolids TS, recorded in WISKI,", currentStart, "to", currentTrimester),
       caption = "South Peel quantities not available from WISKI")+
   coord_flip()+scale_x_discrete(limits = rev)

p_wiskiHaulPlantDry

ggsave(file=paste("export/", currentIssue, "/p_haulageByPlantDry.svg", sep=""), plot=p_wiskiHaulPlantDry, width=16, height=16)
```
