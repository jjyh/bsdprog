...
tbl_maximoAssets_wwSolids<-dt_maximoAssets_wwSolids %>%
  select(loc_code, loc_name, matched_keyword) %>%
  arrange(loc_name, matched_keyword) %>%
  datatable(
    filter = 'top',
...
tbl_maximoAssets_wwSolids
```


```{r, eval = FALSE}
# Create a gt version of the same table - doesn't work on sharepoint (bc of interactivity?)
gt_dt_maximoAssets_wwSolids<-dt_maximoAssets_wwSolids %>%
  select(loc_code, loc_name, matched_keyword) %>%
  arrange(loc_name, matched_keyword) %>%
  mutate(loc_code = as.character(loc_code)) %>%
  gt() %>%
...
basicMap<-leaflet(dt_pcsFacilities) %>% addProviderTiles("CartoDB.Positron") %>%
...
#number of plants utilizing end destination type
```
